...
const TAB_ORDER: TabValue[] = ["summary", "fulltext", "timestamp", "custom", "log"]
...
interface ModeResultTabsProps {
  ...
  onEditCustom: () => void
  /** session 级 prepare 失败时重试：回首页重新提交同一 URL */
  onRetryPrepare?: () => void
}
...
export function ModeResultTabs({
  ...
  const [activeMode, setActiveMode] = useState<TabValue>(initialMode)
  const [markdown, setMarkdown] = useState<string | null>(null)
  // 音频播放：whisper 产物 .wav（asset 协议加载）
  const [audioSrc, setAudioSrc] = useState<string | null>(null)
  const [audioOpen, setAudioOpen] = useState(false)

  // log Tab 无 mode entry；其余取对应模式状态
  const entry = activeMode === "log" ? undefined : session.modes[activeMode]

  // active 模式完成后从磁盘读回产物（截图相对路径 → asset 绝对路径，供预览加载）
  useEffect(() => {
    if (activeMode === "log" || entry?.status !== "done") {
      setMarkdown(null)
      return
    }
    let active = true
  ...
  return (
    <>
      <Tabs value={activeMode} onValueChange={(v) => setActiveMode(v as TabValue)} className="w-full">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <TabsList>
        {TAB_ORDER.map((tab) => {
          const isLog = tab === "log"
          const status = isLog ? "pending" : (session.modes[tab as PromptMode]?.status ?? "pending")
          const active = activeMode === tab
          return (
            <TabsTrigger key={tab} value={tab} className={active ? "" : "text-muted"}>
              {!isLog && status === "running" && <Loader2 className="mr-1 size-3 animate-spin" />}
              {TAB_LABEL[tab]}
              {!isLog && status === "done" && <span className="ml-1 size-1.5 rounded-full bg-success" />}
            </TabsTrigger>
          )
        })}
        </TabsList>
      ...
      {TAB_ORDER.filter((t) => t !== "log").map((mode) => (
      ...
      <TabsContent value="log">
        <SessionLogPanel runId={session.run_id} />
      </TabsContent>
      </Tabs>
...
function ModeBody({
  ...
  // session 级 prepare 失败：transcript 未就绪，所有 mode 都无法生成；显示报错 + 诊断 ID，引导重新提交
  if (session.status === "error" && session.error) {
    return (
      <div className="raised-card p-10 text-center">
        <div className="flex flex-col items-center gap-3">
          <p className="text-sm text-error">{session.error}</p>
          {session.diagnosticId && (
            <p className="text-xs text-muted-soft">诊断 ID：{session.diagnosticId}</p>
          )}
          {onRetryPrepare && (
            <Button variant="secondary" size="sm" onClick={onRetryPrepare}>
              <RotateCcw />
              重新生成
            </Button>
          )}
